...
def main():
    ...
    if not args.resume:
        raise ValueError('Evaluation requires --resume /path/to/checkpoint')
    ...
    _logger.info("Hardcoded attention: %s", type(model.block[0].attn).__name__)
    ...
    # Evaluation needs only the validation split.
    ...
    if args.rank == 0:
        ...
        output_dir = get_outdir(args.output if args.output else './output/train', exp_name, run_time)
