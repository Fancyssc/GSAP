...
parser.add_argument('-c', '--config', default=os.path.join(os.path.dirname(__file__), 'imagenet.yml'), type=str, metavar='FILE',
                    help='YAML config file specifying default arguments')

parser = argparse.ArgumentParser(description='PyTorch ImageNet Training')

# Model detail
parser.add_argument('--model', default='spiking_gsap', type=str, metavar='MODEL',
...
parser.add_argument('--depths', type=int, default=8, help='Number of transformer blocks')
parser.add_argument('--backend', choices=('cupy', 'torch'), default='cupy', help='LIF execution backend')
...
parser.add_argument('--data-dir', '-data-dir', metavar='DIR', default='./data/imagenet',
...
parser.add_argument('--lr-base-size', type=int, default=0, help='Reference global batch for linear peak-LR scaling; 0 uses --lr directly')
...
def main():
    ...
    if args.distributed:
        ...
        _logger.info(
            ...
            f'Process {args.rank}, total {args.world_size}, local_rank {args.local_rank}.'
        )
    else:
    ...
    # Keep args.yaml reusable: do not scale an already-scaled LR on restart.
    ...
    model = create_model(
        ...
        pretrained=False,
        drop_rate=args.drop,
        drop_path_rate=args.drop_path or 0.,
        ...
        T=args.time_step, backend=args.backend,
    )

    args.attention_class = type(model.block[0].attn).__name__
    _logger.info('Hardcoded attention: %s', args.attention_class)
    args_text = yaml.safe_dump(dict(vars(args), lr=args.lr_unscaled, lr_effective=args.lr),
                               default_flow_style=False)
    ...
    if args.rank == 0:
        ...
        dataset_name = _safe_path_name(args.dataset)
        model_name = _safe_path_name(safe_model_name(args.model))
        exp_name = _safe_path_name(exp_name)
        output_dir = get_outdir(
            args.output if args.output else './output/train',
            dataset_name, model_name, exp_name, run_time)
