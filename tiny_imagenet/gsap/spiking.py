...
class GSAPAttention(nn.Module):
    """GSAP attention with the same single-width axial structure as CIFAR."""

    def __init__(self, dim, feature_size=16, backend='cupy'):
        super().__init__()
        ...
        self.feature_conv = nn.Conv2d(dim, self.inner_dim, kernel_size=1, bias=False)
        self.feature_bn = nn.BatchNorm2d(self.inner_dim)
        ...
        self.row_conv = nn.Conv2d(self.inner_dim, self.inner_dim, kernel_size=(1, kernel_size), padding=(0, feature_size - 1), groups=self.inner_dim, bias=False)
        self.row_bn = nn.BatchNorm2d(self.inner_dim)
        self.row_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend=backend)
        # Depthwise column propagation, matching the CIFAR GSAP structure.
        self.column_conv = nn.Conv2d(self.inner_dim, dim, kernel_size=(kernel_size, 1), padding=(feature_size - 1, 0), groups=dim, bias=False)
    ...
    def forward(self, x):
        ...
        feature = self.feature_bn(self.feature_conv(spike)).reshape(T, B, self.inner_dim, H, W)
        feature = self.feature_lif(feature).flatten(0, 1)

        message = self.row_bn(self.row_conv(feature)).reshape(T, B, self.inner_dim, H, W)
        message = self.row_lif(message + feature.reshape(T, B, self.inner_dim, H, W)).flatten(0, 1)
...
class SpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, feature_size=16, backend='cupy'):
        super().__init__()

        attention = GSAPAttention
...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=64, img_size_w=64, patch_size=4, in_channels=3, embed_dims=384, backend='cupy'):
        super().__init__()
        self.image_size = [img_size_h, img_size_w]
        patch_size = to_2tuple(patch_size)
        if patch_size != (4, 4):
            raise ValueError('The CIFAR-style tokenizer has fixed stride 4; patch_size must be 4')
        if min(img_size_h, img_size_w) < 1 or embed_dims < 8 or embed_dims % 8:
            raise ValueError('Image sizes must be positive and embed_dims a multiple of 8')
        self.patch_size = patch_size
        self.C = in_channels
        self.H, self.W = (img_size_h + 3) // 4, (img_size_w + 3) // 4
    ...
    def forward(self, x):
        ...
        x = self.block3_bn(x).reshape(T, B, -1, (H + 1) // 2, (W + 1) // 2)

        x = self.block4_lif(x).flatten(0, 1)
        x = self.block4_mp(x)
        x = self.block4_conv(x)
        x = self.block4_bn(x).reshape(T, B, -1, (H + 3) // 4, (W + 3) // 4)
...
class vit_snn(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        if depths < 1 or T < 1 or mlp_ratios <= 0:
            raise ValueError('depths, T and mlp_ratios must be positive')
        if any(v not in (None, 0, 0.) for v in (drop_rate, attn_drop_rate, drop_path_rate, drop_block_rate)):
            raise ValueError('Use zero dropout/drop_path, matching the effective CIFAR baseline')
        ...
        patch_embed = SpikingTokenizer(img_size_h=img_size_h,
                          ...
                          embed_dims=embed_dims, backend=backend)
        num_patches = patch_embed.num_patches
        block = nn.ModuleList([SpikingTransformer(
            dim=embed_dims, num_heads=num_heads, mlp_ratio=mlp_ratios, qkv_bias=qkv_bias,
            qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[j],
            norm_layer=norm_layer, sr_ratio=sr_ratios, feature_size=max(patch_embed.H, patch_embed.W), backend=backend)
