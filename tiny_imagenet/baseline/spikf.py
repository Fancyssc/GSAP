...
class AdaptiveBinarySimilarity2_1(nn.Module):
    ...
    def __init__(self, num_heads, init_alpha=0.0, init_beta_q=0.0, init_beta_k=0.0, init_gamma=0.0, backend='cupy'):
        super().__init__()
    ...
    def forward(self, q, k):
        ...
        gamma = self.gamma.tanh() *(scale_q + scale_k)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # Forward intentionally omits LayerNorm, matching the CIFAR backbone.
        # Keep checkpoint keys, but exclude these unused weights from DDP.
...
@register_model
def spikf_baseline(pretrained=False, **kwargs):
    if pretrained:
        raise ValueError('No pretrained Tiny ImageNet weights are provided')
    kwargs.pop('drop_block_rate', None)
    kwargs.pop('pretrained_cfg_overlay', None)
    backend = kwargs.pop('backend', 'cupy')
    defaults = dict(img_size_h=64, img_size_w=64, patch_size=4,
                    in_channels=3, num_classes=200, embed_dims=384,
                    num_heads=12, mlp_ratios=4, depths=4, sr_ratios=1, T=4)
    defaults.update(kwargs)
    model = spikformer(backend=backend, **defaults)
