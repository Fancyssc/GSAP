...
from baseline import spikformer  # register spikf_baseline and Spikformer
from baseline import spiking as spiking_baseline  # register spiking_baseline and Spikingformer
...
def parse_args():
    ...
    parser.add_argument('--dataset', default='ucf101dvs', help='dataset')
    parser.add_argument('--num-classes', type=int, default=101, metavar='N',
                        help='number of label classes (default: 101)')
    parser.add_argument('--data-path', default='./data/UCF101DVS', help='dataset')
    ...
    parser.add_argument('--output-dir', default='./runs/ucf', help='path where to save')
    ...
    parser.add_argument('--wandb-mode', default='online', choices=('online', 'offline', 'disabled'),
                        help='wandb run mode')
    parser.add_argument('--T', '--frames-number', dest='T', default=10, type=int, help='number of event frames')
    parser.add_argument('--backend', default='cupy', choices=('cupy', 'torch'),
                        help='LIF backend for GSAP and baseline models')
...
def load_data(dataset_dir, distributed, T):
    print('Loading BrainCog UCF101-DVS (existing train/test split)')
    dataset_train, dataset_test = build_datasets(dataset_dir, T)
...
def main(args):
    ...
    if not is_model(args.model):
        raise ValueError(f'Unknown model: {args.model!r}. Available local models: '
                         'spiking_gsap, qk_gsap, qk_baseline, QKFormer, sdt_baseline, SDT, spikf_baseline, Spikformer, spiking_baseline, Spikingformer')
    ...
    model = create_model(
        ...
        **(dict(T=args.T, num_classes=args.num_classes, backend=args.backend)
           if args.model in ('spiking_gsap', 'qk_gsap', 'qk_baseline', 'QKFormer', 'sdt_baseline', 'SDT', 'spiking_baseline', 'Spikingformer') else {}),
        **(dict(num_classes=args.num_classes)
           if args.model in ('spikf_baseline', 'Spikformer') else {}),
        **(dict(
    ...
    train_snn_aug = None  # Applied once by the dataset, following TIM
    train_trivalaug = None  # TIM UCF default: horizontal flip only
    mixup_fn = None
    mixup_active = args.mixup > 0 or args.cutmix > 0. or args.cutmix_minmax is not None
    if mixup_active:
        mixup_args = dict(
