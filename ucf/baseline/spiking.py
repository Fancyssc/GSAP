...
__all__ = ['spiking_baseline', 'Spikingformer']
tau_thr = 1.5
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., backend='cupy'):
        ...
        self.mlp1_lif = MultiStepLIFNode(tau=tau_thr, detach_reset=True, backend=backend)
        self.mlp1_conv = nn.Conv2d(in_features, hidden_features, kernel_size=1, stride=1)
        self.mlp1_bn = nn.BatchNorm2d(hidden_features)

        self.mlp2_lif = MultiStepLIFNode(tau=tau_thr, detach_reset=True, backend=backend)
...
class SpikingSelfAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, backend='cupy'):
        super().__init__()
        ...
        self.proj_lif = MultiStepLIFNode(tau=tau_thr, detach_reset=True, backend=backend)

        self.q_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.q_bn = nn.BatchNorm1d(dim)
        self.q_lif = MultiStepLIFNode(tau=tau_thr, detach_reset=True, backend=backend)

        self.k_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.k_bn = nn.BatchNorm1d(dim)
        self.k_lif = MultiStepLIFNode(tau=tau_thr, detach_reset=True, backend=backend)

        self.v_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.v_bn = nn.BatchNorm1d(dim)
        self.v_lif = MultiStepLIFNode(tau=tau_thr, detach_reset=True, backend=backend)

        self.attn_lif = MultiStepLIFNode(tau=tau_thr, v_threshold=0.5, detach_reset=True, backend=backend)
...
class SpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, backend='cupy'):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = SpikingSelfAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
                                           attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio, backend=backend)
...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256, backend='cupy'):
        ...
        self.proj1_lif = MultiStepLIFNode(tau=tau_thr, detach_reset=True, backend=backend)
        ...
        self.proj2_lif = MultiStepLIFNode(tau=tau_thr, detach_reset=True, backend=backend)
        ...
        self.proj3_lif = MultiStepLIFNode(tau=tau_thr, detach_reset=True, backend=backend)
        ...
        self.proj4_lif = MultiStepLIFNode(tau=tau_thr, detach_reset=True, backend=backend)
...
class vit_snn(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=4, pretrained_cfg=None, backend='cupy',
                 ):
        super().__init__()
        ...
        block = nn.ModuleList([SpikingTransformer(
            ...
            norm_layer=norm_layer, sr_ratio=sr_ratios, backend=backend)
...
@register_model
def spiking_baseline(pretrained=False, cache_dir=None, **kwargs):
    if pretrained:
        raise ValueError('No pretrained weights are provided for spiking_baseline')
    kwargs.pop('pretrained_cfg_overlay', None)
    drop_block_rate = kwargs.pop('drop_block_rate', None)
    if drop_block_rate not in (None, 0.):
        raise ValueError('Spikingformer does not implement DropBlock')
    config = dict(
    ...
    model = vit_snn(**config)
    model.default_cfg = _cfg(num_classes=model.num_classes, input_size=(2, 128, 128))
    return model
...
def Spikingformer(pretrained=False, **kwargs):
    ...
    return spiking_baseline(pretrained=pretrained, **kwargs)
