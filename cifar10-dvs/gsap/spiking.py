"""CIFAR10-DVS 2-256 adaptation of GSAP (single hardcoded entrypoint).
...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=16, in_channels=2, embed_dims=256, backend='cupy'):
        ...
        if patch_size != (16, 16):
            raise ValueError('CIFAR10-DVS tokenizer has four stride-2 pools; patch_size must be 16')
...
class vit_snn(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=16,
                 in_channels=2, num_classes=10, embed_dims=256, num_heads=16,
...
def spiking_gsap(pretrained=False, cache_dir=None, **kwargs):
    if pretrained:
        raise ValueError('No pretrained CIFAR10-DVS GSAP weights are provided; load your own checkpoint')
