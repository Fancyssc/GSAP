"""Local backbone snapshot from cifar10/gsap/qk.py."""
import torch
...
class GSAPAttention(nn.Module):
    def __init__(self, dim, feature_size=8, lif_backend='torch'):
        super().__init__()
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., lif_backend='torch'):
        super().__init__()
...
class TokenSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, feature_size=8, lif_backend='torch'):
        super().__init__()
...
class PatchEmbedInit(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256, lif_backend='torch'):
        super().__init__()
...
class spiking_transformer(nn.Module):
    def __init__(self,
                 ...
                 pretrained_cfg_overlay=None, drop_block_rate=None, lif_backend='torch',
                 ):
        super().__init__()
