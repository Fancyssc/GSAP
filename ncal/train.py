...
from baseline import spikformer  # register spikf_baseline
...
def parse_args():
    ...
    parser.add_argument('--dataset', default='ncaltech101', help='dataset')
    parser.add_argument('--num-classes', type=int, default=101, metavar='N',
                        help='number of label classes (default: 101)')
    parser.add_argument('--data-path', default='./data/NCaltech101', help='dataset')
    ...
    parser.add_argument('--output-dir', default='./runs/ncal', help='path where to save')
...
def split_to_train_test_set(train_ratio: float, origin_dataset: torch.utils.data.Dataset, num_classes: int, random_split: bool = False):
    ...
    for i, y in enumerate(origin_dataset.targets):
...
def resize_event_frames(frames):
    # Resize each [T, 2, H, W] sample before batching.
    frames = torch.as_tensor(frames, dtype=torch.float32)
    return torch.nn.functional.interpolate(
        frames, size=(128, 128), mode='bilinear', align_corners=False)
...
def load_data(dataset_dir, distributed, T):
    ...
    origin_set = NCaltech101(
        root=dataset_dir, data_type='frame', frames_number=T,
        split_by='number', transform=resize_event_frames)
    if len(origin_set.classes) != 101:
        raise ValueError(f'Expected 101 classes, found {len(origin_set.classes)}. Check dataset folders.')
...
def main(args):
    ...
    if not is_model(args.model):
        raise ValueError(f'Unknown model: {args.model!r}. Available local models: '
                         'spiking_gsap, qk_gsap, spiking_baseline, Spikingformer, spikf_baseline, qk_baseline, QKFormer, sdt_baseline, SDT')
    ...
    model = create_model(
        ...
        **(dict(T=args.T, num_classes=args.num_classes)
           if args.model in ('spiking_baseline', 'Spikingformer', 'spikf_baseline') else {}),
        **(dict(
