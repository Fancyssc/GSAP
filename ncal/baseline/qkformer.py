...
class TokenSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.tssa = Token_QK_Attention(dim, num_heads, backend=backend)
...
class SpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, backend='cupy'):
        super().__init__()
        self.ssa = Spiking_Self_Attention(dim, num_heads, backend=backend)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features= dim, hidden_features=mlp_hidden_dim, drop=drop, backend=backend)
...
class vit_snn(nn.Module):
    def __init__(self,
                 ...
                 embed_dims=256, num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=4, pretrained_cfg=None, in_chans = 3, no_weight_decay = None, backend='cupy'
                 ):
        super().__init__()
        ...
        patch_embed1 = PatchEmbedInit(img_size_h=img_size_h,
                                       ...
                                       embed_dims=embed_dims // 2, backend=backend)
...
@register_model
def qk_baseline(pretrained=False, cache_dir=None, **kwargs):
    if pretrained:
        raise ValueError('No pretrained weights are provided for qk_baseline')
    # timm metadata / unsupported optional regularizer.
    ...
    config = dict(patch_size=16, embed_dims=256, num_heads=16, mlp_ratios=1,
...
def QKFormer(pretrained=False, **kwargs):
    ...
    return qk_baseline(pretrained=pretrained, **kwargs)
