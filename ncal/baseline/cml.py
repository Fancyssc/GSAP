...
class SpikingTokenizer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.maxpool1(x).reshape(T, B, -1, 64, 64).contiguous()

        x = self.proj1_lif(x).flatten(0,1).contiguous()
        x = self.proj1_conv(x)
        x = self.proj1_bn(x)

        x = self.maxpool2(x).reshape(T, B, -1, 32, 32).contiguous()

        x = self.proj2_lif(x).flatten(0, 1).contiguous()
        x = self.proj2_conv(x)
        x = self.proj2_bn(x)

        x = self.maxpool3(x).reshape(T, B, -1, 16, 16).contiguous()
...
@register_model
def cml_baseline(pretrained=False, **kwargs):
    model = vit_snn(
