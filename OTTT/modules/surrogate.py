...
def check_manual_grad(primitive_function, spiking_function, eps=1e-5):
    '''
    :param primitive_function: the primitive function of the surrogate function
    :type primitive_function: callable
    :param spiking_function: the surrogate function
    :type spiking_function: callable
    :param eps: the maximum error
    :type eps: float

    The backward of the surrogate function is usually manually written. This function can be used to check whether the manual gradient is correct.

    It checks that the backward of ``spiking_function`` agrees with that of ``primitive_function``. "Agree" is defined as: their difference does not exceed ``eps``.
