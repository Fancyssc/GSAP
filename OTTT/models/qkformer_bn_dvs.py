...
class TokenSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.tssa = Token_QK_Attention(
            dim, num_heads, single_step_neuron=single_step_neuron, **kwargs)
...
class SpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 single_step_neuron: callable = None, **kwargs):
        super().__init__()
        self.ssa = Spiking_Self_Attention(
            dim, num_heads, single_step_neuron=single_step_neuron, **kwargs)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features=dim, hidden_features=mlp_hidden_dim, drop=drop,
                       single_step_neuron=single_step_neuron, **kwargs)
...
class PatchEmbedInit(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4,
        ...
        self.proj2_lif = single_step_neuron(**kwargs)
        ...
        if kwargs.get('grad_with_rate', False):
            if isinstance(self.proj1_lif, PPPropLIFNode):
                self.proj1_conv = WrapedSNNOp(
                    self.proj1_conv, self.proj1_lif,
                    post_op=nn.Sequential(self.proj1_bn, self.maxpool1))
                self.proj2_conv = WrapedSNNOp(
                    self.proj2_conv, self.proj2_lif,
                    post_op=nn.Sequential(self.proj2_bn, self.maxpool2))
                self.proj3_conv = WrapedSNNOp(
                    self.proj3_conv, self.proj3_lif,
                    post_op=nn.Sequential(self.proj3_bn, self.maxpool3))
            else:
                self.proj1_conv = WrapedSNNOp(self.proj1_conv)
                self.proj2_conv = WrapedSNNOp(self.proj2_conv)
                self.proj3_conv = WrapedSNNOp(self.proj3_conv)
    ...
    def forward(self, x, **kwargs):
        ...
        if not getattr(self.proj1_conv, 'includes_post_op', False):
            x = self.proj1_bn(x)
            x = self.maxpool1(x)
        ...
        x = call_op(self.proj2_conv, x, require_wrap)
        if not getattr(self.proj2_conv, 'includes_post_op', False):
            x = self.proj2_bn(x)
            x = self.maxpool2(x)
        x = self.proj2_lif(x, **kwargs)

        x = call_op(self.proj3_conv, x, require_wrap)
        if not getattr(self.proj3_conv, 'includes_post_op', False):
            x = self.proj3_bn(x)
            x = self.maxpool3(x)
...
class spiking_transformer(nn.Module):
    def __init__(self,
                 img_size_h=128, img_size_w=128, patch_size=4, in_channels=3, num_classes=10,
    ...
    def forward(self, x, **kwargs):
        ...
        x = call_op(self.head, x, require_wrap)
...
def online_qkformer_dvs(pretrained=False, progress=True,
                          single_step_neuron: callable = None, **kwargs):
    return _qkformer(
        embed_dims=256, depths=2,
        single_step_neuron=single_step_neuron, **kwargs)
