...
class SpikingTransformer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        attention_factory = kwargs.pop("attention_factory", SpikingSelfAttention)
...
class SpikingTokenizer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if kwargs.get("grad_with_rate", False):
            self.block1_conv = WrapedSNNOp(self.block1_conv, self.block1_lif)
            self.block2_conv = WrapedSNNOp(self.block2_conv, self.block2_lif)
            if isinstance(self.block3_lif, PPPropLIFNode):
                self.block3_conv = WrapedSNNOp(
                    self.block3_conv, self.block3_lif,
                    post_op=nn.Sequential(self.block3_bn, self.block3_mp))
                self.block4_conv = WrapedSNNOp(
                    self.block4_conv, self.block4_lif,
                    post_op=nn.Sequential(self.block4_bn, self.block4_mp))
            else:
                # OTTT keeps the original call graph: only the conv's spike input is rerouted to the rate.
                self.block3_conv = WrapedSNNOp(self.block3_conv)
                self.block4_conv = WrapedSNNOp(self.block4_conv)
...
class vit_snn(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if not isinstance(depths, int) or depths < 1:
    ...
    def _init_weights(self, module):
        if isinstance(module, (nn.Conv1d, nn.Conv2d)):
            nn.init.kaiming_normal_(
                module.weight, mode="fan_in" if module.groups == module.in_channels else "fan_out", nonlinearity="relu"
            )
...
def online_spikformer_cifar(
    pretrained=False, progress=True, single_step_neuron: callable = None, **kwargs
):
    return _spikingformer(
        embed_dims=kwargs.pop("embed_dims", 384), depths=kwargs.pop("depths", 4), single_step_neuron=single_step_neuron, **kwargs
    )
