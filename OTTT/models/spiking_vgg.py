...
__all__ = [
    'OnlineSpikingVGG', 'online_spiking_vgg11', 'online_spiking_vgg11_ws', 'online_spiking_vgg11f_ws','online_spiking_vgg11_BN',
]
        ...
        if light_classifier:
            ...
            if self.grad_with_rate:
                self.classifier = SequentialModule(
                    ...
                    WrapedSNNOp(nn.Linear(512*(self.fc_hw**2), num_classes)),  # readout: drives no neuron
                )
            else:
                self.classifier = SequentialModule(
                    single_step_neuron, # not in the module, but parameter
            ...
            if self.grad_with_rate:
                n1 = single_step_neuron(**kwargs, neuron_dropout=0.0)
                n2 = single_step_neuron(**kwargs, neuron_dropout=0.0)
                self.classifier = SequentialModule(
                    single_step_neuron,
                    WrapedSNNOp(ScaledWSLinear(512 * 7 * 7, 4096), n1),
                    n1,
                    Scale(2.74),
                    nn.Dropout(),
                    WrapedSNNOp(ScaledWSLinear(4096, 4096), n2),
                    n2,
                    Scale(2.74),
                    nn.Dropout(),
                    WrapedSNNOp(nn.Linear(4096, num_classes)),  # readout: drives no neuron
                )
            else:
                self.classifier = SequentialModule(
                    single_step_neuron,
        ...
        for v in cfg:
            if v == 'M':
                ...
                if use_stride_2:
                    ...
                    stride = 1
                # build the neuron first so it can be handed to the conv that drives it (pp-prop needs it for eps_f)
                n = neuron(**kwargs)
                ...
                if first_conv:
                    first_conv = False
                    if isinstance(n, PPPropLIFNode):
                        # the first conv consumes the raw image, not spikes, so there is
                        # no rate to reroute (OTTT leaves it unwrapped), but it still
                        # drives n, so under pp-prop it is still wrapped to obtain eps_f
                        conv2d = WrapedSNNOp(conv2d, n, wrap=False)
                elif grad_with_rate:
                    conv2d = WrapedSNNOp(conv2d, n)
                if weight_standardization:
                    layers += [conv2d, n, Scale(2.74)]
                elif BN:
                    layers += [conv2d, nn.BatchNorm2d(v), n]
                else:
                    layers += [conv2d, n, Scale(2.74)]
                in_channels = v
...
# using BN requires weight_std = False & BN = True
def online_spiking_vgg11_BN(pretrained=False, progress=True, single_step_neuron: callable = None, **kwargs):
    return _spiking_vgg('vgg11', 'A', False, pretrained, progress, single_step_neuron, BN=True, **kwargs)
...
class OnlineSpikingVGGF(nn.Module):
    ...
    def __init__(self, cfg, weight_standardization=True, num_classes=1000, init_weights=True,
        ...
        if self.grad_with_rate:
            # the feedback conv's output is added to conv1's before entering the neuron, so it cannot be hooked into eps_f
    ...
    def make_layers(cfg, weight_standardization=True, neuron: callable = None, BN=False, **kwargs):
        ...
        for v in cfg:
            if v == 'M':
                ...
                if weight_standardization:
                    if first_conv:
                        # conv1 lives outside the Sequential and enters the neuron only after being added to the feedback, so it cannot be hooked into eps_f
